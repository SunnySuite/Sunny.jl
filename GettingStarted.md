# Getting started with Julia for users of  _Sunny_, the spin dynamics simulator

## Julia basics

This section presents a tutorial for those new to the Julia programming language.
...
The first step is to install [Julia](https://julialang.org/). Download the latest stable release (1.6 as of Sept 2021) appropriate to your system from the [downloads page](https://julialang.org/downloads/). Inside the package is a Julia executable `julia` which launches the Julia terminal. Follow the [platform specific instructions](https://julialang.org/downloads/platform/) to make the path to `julia` known to your system.
 ...
 Executing the command `julia` from the terminal should bring you to a command prompt like this:
...
This interactive mode is called the REPL, short for read-eval-print-loop. You can enter commands here,
```julia
julia> exp(π * im)
-1.0 - 1.2246467991473532e-16im
```
which verifies Euler's identity `e^{π i} = -1` up to a negligible imaginary term of order `10^-16`.

Those who like to drink from the firehose can work through some Julia examples at [_Learn X in Y minutes, where X=Julia_](https://learnxinyminutes.com/docs/julia/). There is also the [official Julia documentation](https://docs.julialang.org/), but you don't need to all of this to get started with Sunny.

Julia has a built-in help facility. From the REPL, enter `?` to enter the help mode, and then enter a function or type to get documentation. For example,
```
help?> exp
...
Math symbols can be accessed with Latex-like notation. For example, typing `\pi` and then hitting Tab will produce `π`. If you see a unicode symbol that you don't know how to type, you can copy-paste it into the help prompt.
...
The next step is to install some Julia packages. You may be familiar with package managers like `pip` and `anaconda` for Python. In Julia this functionality is built in.
...
```
adds three packages. These packages will be useful to users of Sunny.

There are a lot of interesting Julia packages available. You can browse a list of _registered_ packages at [juliapackages.com](https://juliapackages.com/), along with their documentation.
...
Next, return to the REPL by hitting the Backspace key and then enter
```julia
using Plots
...
plot(xs, sin.(xs) ./ xs) 
```
This should eventually bring up a plotting window. The "time to first plot" should be of order 20 seconds, and is something the Julia developers are working to improve.

An exciting new plotting package, [Makie](https://github.com/JuliaPlots/Makie.jl), is currently under heavy development. It supports interactive 3D graphics and customizable GUI elements like buttons and sliders. Install with
```
pkg> add GLMakie
pkg> test GLMakie
```
There are still rough edges in Makie, so it's a good idea to check that these tests pass. As of Sept. 2021, one annoyance is that it time-to-first-plot is about a minute. Important progress will be made in [this refactoring](https://github.com/JuliaPlots/Makie.jl/pull/1085). There are also some [longstanding bugs on Mac](https://github.com/JuliaPlots/Makie.jl/milestone/4).

For producing publication quality plots, it's still hard to beat Matplotlib from Python. Some people are using Makie. Other Julia options include [PGFPlotsX](https://github.com/KristofferC/PGFPlotsX.jl) (which generates Latex directly) and [VegaLite](https://github.com/queryverse/VegaLite.jl) (a Julia interface to the Javascript library [Vega-Lite](https://vega.github.io/vega-lite/)). There also exists a [Julia interface to Matplotlib](https://github.com/JuliaPy/PyPlot.jl), but it is probably easier to just use Python directly.

## Notebook environments

As seen above, one can execute commands directly from the built-in REPL in a terminal window. Another way to interact with Julia is through the Jupyter notebook interface, which may be familiar to Python users. One can integrate Julia with the Jupyter notebook system through the [IJulia](https://github.com/JuliaLang/IJulia.jl) package. Install it like usual from the package manager,
...
Instead of Git terminal commands, it's often much nicer to user a graphical user interface. For Julia development, I would highly recommend the VSCode editor, described below. VSCode has decent built-in support for Git. It can also be useful to have a Git GUI such as [SmartGit](https://www.syntevo.com/smartgit/), which free for open source use.
...
VSCode does bring a significant learning curve. The Command Palette can help a lot with discoverability. Access it through the `View -> Command Palette...` menu item (Shift-Command-P on Mac, ??? on Linux/Windows). Here you can start typing a command to get a list of suggestions. For example, entering "julia repl" will suggest the `Julia: Start REPL` command. Hit Enter to launch a Julia REPL, which will appear at the bottom of the VSCode window. This running Julia process is integrated with other Julia-VSCode features in a powerful way.

Again bring up the Command Palette (`View -> Command Palette...`) and type "settings json" to find the command `Preferences: Open Settings (JSON)`. Hit enter to and you will an editor for the `Settings.json` file. Add to the bottom of this file the line
...
It is frequently useful to launch VSCode from the terminal. To enable this, run `Shell Command: Install 'code' command in PATH` using the Command Palette. As expected, this will create a shell command `code` that can be used to quickly launch VSCode from the terminal. The usage `code <filename>` and `code <directory>` is also supported.

You may wish to make VSCode the default editor. On a UNIX system, this is possible by adding the line
```bash
export EDITOR=code
```
to the shell startup script (e.g. `.bashrc` or `.bash_profile`).

This `EDITOR` environment variable will be used by the Julia `@edit` command. For example, running
```julia
julia> @edit sort([3, 2, 1])
```
will open the source code for the built-in `sort` function within VSCode. Besides `@edit`, Julia supports several other [interactive utilities](https://docs.julialang.org/en/v1/stdlib/InteractiveUtils/).
