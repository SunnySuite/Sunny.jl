...
function Sunny.export_vtk(filename, sys; coordinates=:physical, log_scale=false)
    ...
    x = [dot([i, j, k], latvecs[1, :]) for i in 1:Ni, j in 1:Nj, k in 1:Nk]
    y = [dot([i, j, k], latvecs[2, :]) for i in 1:Ni, j in 1:Nj, k in 1:Nk]
    z = [dot([i, j, k], latvecs[3, :]) for i in 1:Ni, j in 1:Nj, k in 1:Nk]

    # SQTODO: Dipoles point in an un-physical direction in :lattice coordinate mode
    dipole_data = [
        [sys.dipoles[i, j, k, l] for i in 1:Ni, j in 1:Nj, k in 1:Nk] for
        l in 1:natoms(sys.crystal)
    ]
    ...
    saved_files_more = export_vtk(
        joinpath(filename, "correlations"), params, signal; dims_kept=[1, 2, 3]
    )
    return append!(saved_files, saved_files_more)
end
...
function Sunny.export_vtk(filename, params::BinningParameters, data; dims_kept=nothing)
        ...
        if isnothing(dims_kept) # Auto-detect which dimensions are singleton
            ...
            for dim in dims_kept
                push!(
                    edges,
                    Sunny.axes_binedges(
                        params.binstart[dim], params.binend[dim], params.binwidth[dim]
                    )[1],
                )
            end
    ...
    centers = [
        SVector{nd,Float64}([bcs[i][ix[i]] for i in 1:nd]) for
        ix in CartesianIndices(size(data))
    ]
