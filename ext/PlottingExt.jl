...
function all_images_within_distance(
    latvecs, rs, r0s; min_dist=0, max_dist, include_zeros=false
)
    ...
    # and k differ from i)
    box_lengths = [
        a ⋅ b / norm(b) for (a, b) in zip(eachcol(latvecs), eachrow(inv(latvecs)))
    ]
...
function Base.show(io::IO, ::MIME"text/plain", fig::NotifiableFigure)
    return print(io, "(Notifiable) " * repr(fig.figure))
end
...
function Makie.record(func, nf::NotifiableFigure, path, iter; kwargs...)
    return Makie.record(func, nf.figure, path, iter; kwargs...)
end
...
function numbers_to_colors!(
    out::AbstractArray{Makie.RGBAf}, in::AbstractArray{<:Number}, colormap, colorrange
)
...
function plot_spins!(
    ...
    if show_cell
        Makie.linesegments!(
            ax, cell_wireframe(orig_crystal(sys).latvecs, dims); color=:teal, linewidth=1.5
        )
    end
    ...
    images = all_images_within_distance(
        supervecs, rs, [r0]; max_dist=ghost_radius, include_zeros=true
    )
    ...
    for isghost in (false, true)
        ...
        Makie.on(notifier; update=true) do _
            ...
            if !isnothing(colorfn)
                ...
                numbers_to_colors!(
                    rgba_colors, numeric_colors, cmap_with_alpha, dyncolorrange
                )
            end
        ...
        Makie.meshscatter!(
            ax, pts; markersize, color=linecolor, diffuse=1.15, transparency=isghost
        )
    end
    ...
    if show_cell
        ...
        pos = [
            (3 / 4) * Makie.Point3f0(p) for p in eachcol(orig_crystal(sys).latvecs)[1:dims]
        ]
...
function Sunny.view_crystal(
    ...
    Makie.linesegments!(
        ax,
        cell_wireframe(cryst.latvecs, dims);
        color=:teal,
        linewidth=1.5,
        inspectable=false,
    )
    ...
    axes = Makie.arrows!(
        ax,
        Makie.Point3f0.(fill([0, 0, 0], 3)),
        axisdirs;
        arrowsize=0.5axissize,
        linewidth=0.15axissize,
    ...
    images = all_images_within_distance(
        cryst.latvecs, cryst.positions, cryst.positions; max_dist, include_zeros=true
    )
    atom_labels = nothing
    ...
    function all_segments_for_bond(b, color, visible)
        ...
        bond_labels = map(bonds) do b′
            ...
            basis_strs = Sunny.coupling_basis_strings(
                zip('A':'Z', basis); digits=12, atol=1e-12
            )
        ...
        s = Makie.linesegments!(
            ax, segments; color, linewidth=3, inspectable=true, inspector_label, visible
        )
        return [s]
    end
    ...
    axes_labels[1, 2] = Makie.Label(
        fig, "x"; color=Makie.RGBf(0.90, 0.0, 0.0), font=:bold, fontsize
    )
    axes_labels[1, 3] = Makie.Label(
        fig, "y"; color=Makie.RGBf(0.90, 0.5, 0.0), font=:bold, fontsize
    )
    axes_labels[1, 4] = Makie.Label(
        fig, "z"; color=Makie.RGBf(0.90, 0.85, 0.0), font=:bold, fontsize
    )
    ...
    layout[toggle_cnt+=1, 1:2] = [
        atom_labels_toggle, Makie.Label(fig, "Show atom indices"; fontsize)
    ]
    ...
    Makie.DataInspector(
        ax; fontsize, font=pkgdir(Sunny, "assets", "fonts", "RobotoMono-Regular.ttf")
    )
        ...
        for i in eachindex(center)
            ...
            for j in eachindex(theta)
                pts[j] =
                    center[i] .+
                    sin(phi) .* radius .*
                    (cos(theta[j]) .* codir1 .+ sin(theta[j]) .* codir2) .+
                    radius .* (m / l) .* dir[i]
            end
            Makie.lines!(
                pts;
                color=Makie.HSVA(rad2deg(angle(z[i][level])), 1, 1, abs2(z[i][level])),
                linewidth,
            )
        end
    end
...
function plot_coherents(
    sys::System{N}; scale=1.0, quantization_axis=nothing, use_arrows=true, size=(768, 512)
) where {N}
    ℓ0 = characteristic_length_between_atoms(orig_crystal(sys))
...
function scatter_bin_centers!(ax, params; axes)
    ...
    for xx in bcs[axes[1]], yy in bcs[axes[2]]
        push!(xs, xx)
        push!(ys, yy)
    end
    return Makie.scatter!(ax, xs, ys; marker='x', markersize=10, color=:black)
end
...
function __init__()
    return Sunny.Plotting = @__MODULE__
end
