...
* When loading a CIF or mCIF, the precision parameter `symprec` becomes optional
  ([#413](@ref)).
* Various enhancements to [`minimize_energy!`](@ref). A small perturbation to
  the initial spin state breaks accidental symmetries ([#442](@ref)). The return
  value becomes a struct that stores optimization statistics ([#430](@ref)).
  Convergence to the local minimum becomes faster and more robust
  ([#453](@ref)).
