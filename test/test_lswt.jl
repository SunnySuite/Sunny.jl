@testitem "Kitchen Sink" begin
    ...
    res = intensities_bands2(swt, qs; measure=DSSF_perp(sys))
    ...
    res = intensities_bands2(swt, qs; measure=DSSF(sys), formfactors)
...
@testitem "Intensities" begin
    ...
    function compute(mode)
        ...
        swt = SpinWaveTheory(sys)
        q = [0.8, 0.6, 0.1]
        res = intensities_bands2(swt, [q]; measure=DSSF_trace(sys))
...
@testitem "General biquadratic" begin
    ...
    res = intensities_bands2(swt, [[0,0,0]]; measure)
    @test res.disp[1] ≈ 9
    @test res.data[1] ≈ 1
end
...
@testitem "SW15-Langasite" begin
    ...
    res = intensities_bands2(swt, [q]; measure)
...
@testitem "3Q Pyrochlore" begin
    ...
    res = intensities_bands2(swt, q; measure=DSSF(sys), formfactors)
...
@testitem "Invariance to reshaping" begin    
    ...
    q = randn(3)
    energies = 0:0.01:6
    res1 = intensities2(swt1, [q]; energies, kernel, measure=DSSF_perp(sys_prim), formfactors)
    res2 = intensities2(swt2, [q]; energies, kernel, measure=DSSF_perp(sys), formfactors)
...
@testitem "Invariance to spin rotation" begin
    ...
    res1 = intensities_bands2(swt, [[0,0,0]]; measure=DSSF_trace(sys1))
    swt = SpinWaveTheory(sys2)
    res2 = intensities_bands2(swt, [[0,0,0]]; measure=DSSF_trace(sys2))
...
@testitem "Spin ladder intensities reference test" begin
    ...
    res = intensities2(swt, qs; energies, kernel, measure)
