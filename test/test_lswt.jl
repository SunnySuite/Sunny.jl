...
@testitem "langasite" begin
  ...
  @testitem "langasite" begin
    ...
    for site ∈ 1:7
        set_dipole!(sys,Rr(site)*S1,(1,1,site,1))
        set_dipole!(sys,Rr(site)*S2,(1,1,site,2))
        set_dipole!(sys,Rr(site)*S3,(1,1,site,3))
    end
    swt = SpinWaveTheory(sys);
    q =  [[0.41568,0.56382,0.76414]];
    ...
    SpinW_intensities_reshape = reshape(SpinW_intensities,(3,3,21))./3 # 3 x 3 x 21 to match static array
    sqdiffs= []
