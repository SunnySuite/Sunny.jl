...
@testiten "langasite" begin
    a = b = 8.539
    c = 5.2414
    latvecs = lattice_vectors(a, b, c, 90, 90, 120)
    langasite = Crystal(latvecs, [[0.24964,0,0.5]], 150)
    latsize = (1,1,7)
    rn_seed = 5
    S = 5/2
    J₁ = 0.85
    ...
    set_dipole!(sys,[-0.5044533423396333, 2.001893342877889, 1.4099467611025476],(1,1,1,1))
    set_dipole!(sys,[0.1293233021090911, 2.417743427310558, -0.6227296389511265],(1,1,2,1))
    set_dipole!(sys,[0.6657168659750873, 1.012983397642032, -2.1864779190419266],(1,1,3,1))
    set_dipole!(sys,[0.70081205063808, -1.1545737917105865, -2.103763729408673],(1,1,4,1))
    set_dipole!(sys,[0.20818146498888318, -2.4527133663679717, -0.4368725444279783],(1,1,5,1))
    set_dipole!(sys,[-0.44121400802104255, -1.9039097506943468, 1.558992578666428],(1,1,6,1))
    set_dipole!(sys,[-0.7583663315317266, 0.07857674101192728, 2.380904492618521],(1,1,7,1))
    set_dipole!(sys,[-0.23941493356376145, -2.2964801847830247, 0.9585714634212639],(1,1,1,2))
    set_dipole!(sys,[-0.7127534363253039, -0.661169840589675, 2.303223172187423],(1,1,2,2))
    set_dipole!(sys,[-0.649374063227625, 1.472014878852633, 1.9135008550935415],(1,1,3,2))
    set_dipole!(sys,[-0.09700277053961495, 2.4967423710456935, 0.08287336805494176],(1,1,4,2))
    set_dipole!(sys,[0.5284135894751638, 1.641371933735445, -1.810159455297665],(1,1,5,2))
    set_dipole!(sys,[0.7559237378214758, -0.44998504790813443, -2.340105288070843],(1,1,6,2))
    set_dipole!(sys,[0.4142078971887959, -2.202494109788788, -1.1079041087802364],(1,1,7,2))
    set_dipole!(sys,[0.7438682759624646, 0.2945868417178887, -2.3685182246930907],(1,1,1,3))
    set_dipole!(sys,[0.5834301340289597, -1.756573586681836, -1.6804935326500545],(1,1,2,3))
    set_dipole!(sys,[-0.016342802533500442, -2.484998276803468, 0.2729770633022944],(1,1,3,3))
    set_dipole!(sys,[-0.6038092802747632, -1.342168580269196, 2.0208903619920218],(1,1,4,3))
    set_dipole!(sys,[-0.7365950542700556, 0.811341433183202, 2.2470319990656837],(1,1,5,3))
    set_dipole!(sys,[-0.3147097298632908, 2.3538947981513774, 0.7811127096426373],(1,1,6,3))
    set_dipole!(sys,[0.3441584343023569, 2.12391737006838, -1.2730003838256954],(1,1,7,3))
    swt = SpinWaveTheory(sys);
    q =  [0.41568,0.56382,0.76414];
    formula = intensity_formula(swt,:full; kernel=delta_function_kernel)
    dispersion, intensities = intensities_bands(swt, q; formula)
    ##### Insert stuff from SpinW
end