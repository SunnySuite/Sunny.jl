@testitem "Kitchen Sink" begin
    ...
    res = Sunny.intensities2(swt, qs; measure=Sunny.DSSF_perp(sys))
    ...
    res = Sunny.intensities2(swt, qs; measure=Sunny.DSSF(sys), formfactors)
...
@testitem "Intensities" begin
    ...
    function compute(mode)
        ...
        swt = SpinWaveTheory(sys)
        q = [0.8, 0.6, 0.1]
        res = Sunny.intensities2(swt, [q]; measure=Sunny.DSSF_trace(sys))
...
@testitem "General biquadratic" begin
    ...
    swt = SpinWaveTheory(sys)
    measure = Sunny.DSSF_trace(sys; apply_g=false)
    res = Sunny.intensities2(swt, [[0,0,0]]; measure)
    @test res.disp[1] ≈ 9
    @test res.data[1] ≈ 1
end
...
@testitem "SW15-Langasite" begin
    ...
    measure = Sunny.DSSF(sys; apply_g=false)
    res = Sunny.intensities2(swt, [q]; measure)
...
@testitem "3Q Pyrochlore" begin
    ...
    res = Sunny.intensities2(swt, q; measure=Sunny.DSSF(sys), formfactors)
...
@testitem "Invariance to reshaping" begin    
    ...
    q = randn(3)
    energies = 0:0.01:6
    res1 = Sunny.intensities_broadened2(swt1, [q], energies; kernel, measure=Sunny.DSSF_perp(sys_prim), formfactors)
    res2 = Sunny.intensities_broadened2(swt2, [q], energies; kernel, measure=Sunny.DSSF_perp(sys), formfactors)
...
@testitem "Invariance to spin rotation" begin
    ...
    res1 = Sunny.intensities2(swt, [[0,0,0]]; measure=Sunny.DSSF_trace(sys1))
    swt = SpinWaveTheory(sys2)
    res2 = Sunny.intensities2(swt, [[0,0,0]]; measure=Sunny.DSSF_trace(sys2))
...
@testitem "Spin ladder intensities reference test" begin
    ...
    res = Sunny.intensities_broadened2(swt, qs, energies; kernel, measure)
