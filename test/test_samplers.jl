...
@testitem "Anisotropy" begin
    ...
    function su5_mean_energy(kT, D)
        a = 4D / (5kT)
        return 4D * (exp(-a) * (-a * (a * (a * (a + 4) + 12) + 24) - 24) + 24) /
               (5a * (exp(-a) * (-a * (a * (a + 3) + 6) - 6) + 6))
    end
    ...
    function asymmetric_crystal()
        ...
        positions = [[0, 0, 0]]
        return Crystal(latvecs, positions, 1)
    end
...
@testitem "Spin chain" begin
    ...
    function cubic_slice_area(α, k)
        return sum([
            (-1)^i * binomial(k, i) * (α - i)^(k - 1) / factorial(k - 1) for
            i in 0:floor(Int, α)
        ])
    end
    ...
    function P(E, kT; n=2, J=1.0)
        E_min = -J * max(1.0, n - 1.0)
        return (2J)^(n - 2) * cubic_slice_area((E - E_min) / 2J, n - 1) * exp(-E / kT) /
               (2kT * sinh(J / kT))^(n - 1)
    end
