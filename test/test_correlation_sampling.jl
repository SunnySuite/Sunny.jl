@testitem "Correlation sampling" begin
    ...
    intensities_symmetric = intensities_interpolated(
        sc, [(0.25, 0.5, 0)], formula_imaginary_parts
    )
    ...
    vals = intensities_interpolated(
        sc, qgrid, intensity_formula(sc, :trace; formfactors); negative_energies=true
    )
    ...
    static_vals = instant_intensities_interpolated(
        sc, qgrid, trace_formula; negative_energies=true
    )
    ...
    add_sample!(ic, sys)
    true_static_vals = instant_intensities_interpolated(
        ic, qgrid, intensity_formula(ic, :trace)
    )
...
@testitem "Merge correlations" begin
    # Set up a system.
    sys = System(
        Sunny.diamond_crystal(), (2, 2, 2), [SpinInfo(1; S=3 / 2, g=2)], :dipole; seed=101
    )
    ...
    sc0 = dynamical_correlations(
        sys; nω=25, ωmax=5.5, Δt=2Δt_langevin, calculate_errors=true
    )
    sc1 = dynamical_correlations(
        sys; nω=25, ωmax=5.5, Δt=2Δt_langevin, calculate_errors=true
    )
    sc2 = dynamical_correlations(
        sys; nω=25, ωmax=5.5, Δt=2Δt_langevin, calculate_errors=true
    )
...
@testitem "Sampled correlations reference" begin
    sys = System(
        Sunny.diamond_crystal(), (3, 3, 3), [SpinInfo(1; S=3 / 2, g=2)], :dipole; seed=101
    )
    set_exchange!(sys, 0.6498, Bond(1, 3, [0, 0, 0]))
    randomize_spins!(sys)
    ...
    data = intensities_interpolated(
        sc, qs, intensity_formula(sc, :trace; kT); interpolation=:linear
    )

    refdata = [
        4.254269966153992 -1.2843038669061228e-14 -5.394619322304545e-15 -7.216011361021513e-15 2.2240831664270433e-15 3.4163924368862625e-15 -2.213918174402409e-15 2.5176941933533583e-15 -1.6967936802985614e-15 -5.809570898878131e-16 9.221497776781295e-17 -1.1157998428324525e-15 -2.379142682899133e-15 2.8471327104209816e-15 1.7428601452501232e-15 -1.2189646774710288e-15 4.2787677309219294e-15 -1.0973563787964497e-15 7.054433862891246e-16 4.380204032382625e-17 1.7981890238059382e-15 8.714300653810433e-16 2.028726078134631e-15 3.1814113498015657e-16 -1.2725645399205768e-15
        0.691817031974635 0.022003904265315233 0.015402770605279703 0.01416546690741492 0.010268650053888165 0.018517150867665515 0.03052522221486546 0.16122718426522498 1.758858093426611 1.477056212473994 0.17592893333848564 0.9125908527099499 0.13542564389015566 0.02791055330977605 0.020926562473942117 0.020478478271579087 0.019894295068977528 0.01461089841500453 0.017161028051175586 0.02342242221257691 0.015976223751103427 0.016509502868504597 0.014721445623676686 0.020611082181934776 0.027610813222342722
        0.008860945396244786 0.027072547009685838 0.02407959453553224 0.03716552552216021 0.026340120173756747 0.03450469651089368 0.029861166686208994 0.18115847147999378 0.9207275891278851 0.9146627283758494 0.43083559486712847 8.309560489648792 2.096790402760553 2.008750574046653 3.2097755895043982 1.929761239453324 0.3246121762953159 0.09985482227045933 0.07389371860037938 0.05252516529923479 0.041887672434665145 0.04800612203272573 0.03783926774429406 0.04171088150600197 0.04288203460674578
    ]
