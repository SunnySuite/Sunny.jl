@testitem "Form factors" begin
    ...
    @test sprint(show, FormFactor("Fe2")) == "FormFactor(\"Fe2\"; config=\"3d⁶\", c_j2=0)"
    @test sprint(show, FormFactor("Fe2"; c_j2=:free_ion)) == "FormFactor(\"Fe2\"; config=\"3d⁶\", c_j2=1/3)"
    ...
    q2 = 0.57
    function check_ffs(keys, refs, configs=fill(nothing, length(keys)); c_j2)
        for (key, config, ref) in zip(keys, configs, refs)
            @test ref ≈ Sunny.compute_form_factor(FormFactor(key; config, c_j2), q2)
        end
    end
    ...
    check_ffs(brown_keys, brown_refs; c_j2=:free_ion)
    ...
    check_ffs(kobayashi_keys, kobayashi_refs, kobayashi_configs; c_j2=:free_ion)
    ...
    check_ffs(brown_singlet_keys, brown_singlet_refs; c_j2=-3/43)
    ...
    check_ffs(kobayashi_singlet_keys, kobayashi_singlet_refs, kobayashi_singlet_configs; c_j2=-3/43)
    ...
    @test_throws "Free-ion Landé factor is ambiguous when J=0 (config 3d¹⁰)" Sunny.compute_form_factor(FormFactor("Cu1"; c_j2=:free_ion), q2)
