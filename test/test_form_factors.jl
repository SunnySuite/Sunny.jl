@testitem "Form factors" begin
    ...
    @test sprint(show, FormFactor("Fe2")) == "FormFactor(\"Fe2\"; config=\"3d⁶\", j2_coefficient=1/3) [term=5D4]"
    ...
    q2 = 0.57
    function check_ffs(keys, refs, configs=fill(nothing, length(keys)); j2_coefficient=nothing)
        for (key, config, ref) in zip(keys, configs, refs)
            @test ref ≈ Sunny.compute_form_factor(FormFactor(key; config, j2_coefficient), q2)
        end
    end
    ...
    check_ffs(kobayashi_keys, kobayashi_refs, kobayashi_configs)
    ...
    check_ffs(kobayashi_singlet_keys, kobayashi_singlet_refs, kobayashi_singlet_configs; j2_coefficient=-3/43)
