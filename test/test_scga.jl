...
@testitem "MgCr2O4" begin
    ...
    S = 3/2
    γ=S*(S+1)*length(cryst.positions)
    @test abs(sum(reshape(res.data,size(MgCrO))/γ - (3/4)*MgCrO)/length(grid.qs))/length(MgCrO) < 1e-5
...
@testitem "Arbitrary Anisotropy" begin
    ...
    S = 1
    γ=S*(S+1)*length(cryst.positions)
    @test abs(sum(reshape(res.data, size(arb))-arb)) / length(grid.qs) < tol
end
...
@testitem "SCGA Kitchen sink" begin
    ...
    res = Sunny.intensities_static(scga, qs; kT, sublattice_resolved=true, tol=1e-10, Nq=10, λs_init)
