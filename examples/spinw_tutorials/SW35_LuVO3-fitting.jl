...
# direction, respectively. The single-ion anisotropy term has the form ``-
# \sum_α K_{αα} S_α^2``. Note that a shift of all ``K_{xx}, K_{yy}, K_{zz}`` by
# some constant ``c`` amounts to a physically irrelevant shift ``- c |S|^2`` of
# the spin Hamiltonian. Without loss of generality, one can select ``K_{zz} =
# 0``. With this convention, negative ``K_{xx}`` and ``K_{yy}`` yields an
...
# Set parameters for phase III following Table I of Skoulatos et al. Here, it
# seems the paper has a typo. To get agreement in the calculated spin wave
# spectrum, it is necessary to swap the values of their reported exchange
# parameters, ``J_{ab} = 4.24`` and ``J_{c} = 5.95`` (meV).
...
# This tutorial refits the model parameters using the dispersion data of Fig. 1d
# of Skoulatos et al. The numbers below were extracted using the
...
# structure is known experimentally to be Néel. Sunny provides
# `labeled_peaks_mismatch` for robust comparison of the predicted bands with the
# experimentally measured intensity peaks. 
...
# independent runs of particle swarm to search for the most promising model
# structure.
