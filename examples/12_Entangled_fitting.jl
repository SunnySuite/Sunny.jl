# # 12. Fitting exchange constants for a dimerized magnet
#
# This tutorial fits the exchange constants of Ba₃Mn₂O₈ to its measured
# singlet-triplet dispersion using Sunny's [entangled-unit](@ref
# entangle-system) formalism. Ba₃Mn₂O₈ is an ``s=1`` bilayer antiferromagnet on
# a triangular lattice with ABC stacking. A strong antiferromagnetic bond
# dimerizes the two spins of each bilayer into a singlet, giving a nonmagnetic
# ground state and gapped triplon excitations. The experimental data comes from
# [Stone et al., Phys. Rev. Lett. **100**, 237201
# (2008)](https://doi.org/10.1103/PhysRevLett.100.237201).

using Sunny, GLMakie, LinearAlgebra

# Ba₃Mn₂O₈ belongs to spacegroup 166 (R3̄m). The magnetic Mn⁵⁺ ions occupy
# Wyckoff position 6c.
...
# Label interactions following Stone et al. Initialize the interaction strengths
# to zero. Later these will be fit to data.
...
# Consider, first, a reduced model with just antiferromagnetic exchange ``J_0``
# between each atom pair (1, 2), (3, 4), and (5, 6). If quantum entanglement is
# neglected, this interaction favors anti-alignment of the spin dipoles on each
# dimer, yielding a total energy of ``-J_0/2`` per site.
...
# Each spin-1 dimer has a singlet ground state ``(|1,-1⟩ - |0,0⟩ +
# |-1,1⟩) / \\sqrt{3}`` with an energy of ``-J_0`` per site. Sunny's
# [`entangle_system`](@ref) feature models this dimerized physics exactly.
...
# To accelerate spin wave calculations, we will work in the rhombohedral
# primitive cell, which contains just one dimer. As a consistency check, verify
# that this reshaping preserves the energy per site.
...
# Now we follow Stone et al. and fit the exchange parameters to the labeled
# intensity peak data appearing in Fig. 3.
...
# depends on only four exchange parameters, ``(J_0, J_1, J_2 - J_3, J_4)``. In
# this limit, we may select ``J_3 = 0`` arbitrarily. Then the effective ``J_2``
# model parameter actually represents ``J_2 - J_3``.

set_params!(esys, [:D, :J3], [-0.032, 0])

# There remain four parameters to be fitted.

labels = [:J0, :J1, :J2, :J4]

# Use [`make_loss_fn`](@ref) to define the optimization target. It expects trial
# values for the four fitting parameters and returns a goodness of fit.
# Internally, [`squared_error_bands`](@ref) assigns each experimentally labeled
# peak in `Es` to the closest spin wave mode in `res` (disallowing duplicated
# assignments). We omit the call to `minimize_energy` because the dimer has
...
@show fit.minimizer # [J0, J1, J2, J4]
...
# | ``J_0``       | 1.65 ± 0.04      | 1.642              |
# | ``J_1``       | 0.12 ± 0.02      | 0.118              |
# | ``J_2 - J_3`` | 0.11 ± 0.01      | 0.256              |
# | ``J_4``       | 0.02 ± 0.02      | 0.037              |
#
# Per the [published erratum](https://doi.org/10.1103/PhysRevLett.105.169901) of
# Stone et al., all fitted exchanges are antiferromagnetic.
#
# Finally, plot the fitted dispersion together with the labeled peaks. Only the
# triplon modes near 1-3 meV carry appreciable intensity. The helper
# [`find_qs_along_path`](@ref) maps each labeled ``𝐪``-point to an index along
# the path, which serves as an ``x``-coordinate in the
# [`plot_intensities`](@ref) scene.
