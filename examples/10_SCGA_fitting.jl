...
# important constraint on the _scale_ of the ``\mathcal{S}(𝐪)``. For
...
# [`SCGA`](@ref) calculator to simulate
...
# The `loss` can be evaluated like a normal function. The initial guess is a
# "null model" without any exchange coupling.
...
# SCGA-predictions for ``\mathcal{S}(𝐪)`` on the slices [H, K, 1/2] and [H, K,
# 1] can be compared with experimental data from Fig. 1d of [Bai et
...
for (i, L) in enumerate((0.5, 1))
    ...
    plot_intensities!(fig[1, i], res, title="SCGA-fitted S(q), L = $L", colorrange=(0, 275))
end
fig
