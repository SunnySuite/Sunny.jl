...
function trajectory(sys::System{N}, Δt, nsnaps, ops; kwargs...) where {N}
    ...
    traj_buf = zeros(
        N == 0 ? Float64 : ComplexF64, num_ops, sys.latsize..., natoms(sys.crystal), nsnaps
    )
...
function symmetrize!(sc::SampledCorrelations)
    ...
    for t in 1:nsteps
        selectdim(samplebuf, 6, t) .=
            0.5 * (selectdim(samplebuf, 6, t) + selectdim(samplebuf, 6, nsteps - t + 1))
    end
end
