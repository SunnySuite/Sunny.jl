mutable struct WangLandau{N,PR}
    ...
    function WangLandau(;
        sys::System{N}, bin_size, bounds, propose::PR, ln_f=1.0, max_sweeps_relax=100
    ) where {N,PR}
        drive_system_to_energy_bounds!(sys, bounds, propose, max_sweeps_relax)
        E = energy_per_site(sys)
        return new{N,PR}(
            sys, Histogram(; bin_size), Histogram(; bin_size), bounds, propose, ln_f, E
        )
    end
end

# use MCMC sampling to get system into bounded energy range
function drive_system_to_energy_bounds!(
    sys::System{N}, bounds, propose::PR, max_sweeps
) where {N,PR}
    E0 = energy_per_site(sys)

    inbounds(E0, bounds) && return nothing
    ...
    for _ in 1:max_sweeps
        ...
        inbounds(E, bounds) && return nothing
    end

    if iszero(kT)
        error(
            "Failed to drive system to energy bounds. Try initializing the system to a low-energy configuration.",
        )
    else
        error(
            "Failed to drive system to energy bounds. The energy scale may be unphysically high.",
        )
    end
end
