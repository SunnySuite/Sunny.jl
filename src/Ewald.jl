...
function energy(dipoles::Array{Vec3, 4}, ewald::EwaldCPU)
    ...
    @tullio U += real(
        conj(Fs[α, j, k, l, b1]) * FA[α, β, j, k, l, b1, b2] * Fs[β, j, k, l, b2]
    )
    return U / prod(sz)
end

# Calculate the field, -dE/ds, at idx1 generated by a dipole at idx2.
function pairwise_field(idx1, idx2, dipole::Vec3, ewald::EwaldCPU)
    sz = size(ewald.ϕ)[1:3]
...
function _accum_neggrad!(H::Array{Vec3, 4}, dipoles::Array{Vec3, 4}, ewald::EwaldCPU)
    (; FA, Fs, Fϕ, ϕ) = ewald

    fill!(Fϕ, 0.0)
    mul!(Fs, ewald.plan, reinterpret(reshape, Float64, dipoles))
    @tullio grad=false Fϕ[α,i,j,k,b1] += FA[α,β,i,j,k,b1,b2] * Fs[β,i,j,k,b2]
