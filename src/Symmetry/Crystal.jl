...
symmetry information is automatically inferred using the Spglib package [1, 2].
The optional parameter `types` is a list of strings, one for each atom, and can
be used to break symmetry-equivalence between atoms.
...
a spacegroup number is provided, the ITA standard setting will be employed [3],
consistent with conventions from the Bilbao crystallographic server [4]. If a
spacegroup symbol is provided that allows for multiple ITA settings, the
possible disambiguations will be included in an error message, and may involve a
`choice` string.
...
1. [A. Togo, K. Shinohara, I. Tanaka, _Spglib: a software library for crystal
   symmetry search_, [arXiv:1809.01590]](https://arxiv.org/abs/1808.01590).
2. The Spglib package, [available online](https://github.com/spglib/spglib).
3. International Tables of Crystallography, [Volume
   A](https://doi.org/10.1107/97809553602060000114).
4. Wyckoff position listings are available from the [Bilbao crystallographic
   server](https://www.cryst.ehu.es/cryst/get_wp.html).
"""
struct Crystal
