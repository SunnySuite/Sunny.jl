...
function conventionalize_setting(latvecs::Mat3, setting::SymOp, sgnum::Int)
    ...
    @assert allunique(scores) "Could not select standard cell unambiguously"
...
function validate_crystal(cryst::Crystal)
    (; latvecs, types, classes, sg) = cryst

    # Multiplicity and type are per-class properties, so check one atom per class
    # rather than redundantly recomputing the (expensive) Wyckoff for every atom.
    for c in unique(classes)
        idxs = findall(==(c), classes)
        # Atoms of the same class must have the same type
        @assert all(==(types[first(idxs)]), @view types[idxs])
        # Wyckoffs must have the correct multiplicity
        w = get_wyckoff(cryst, first(idxs))
        @assert w.multiplicity ≈ length(idxs) / abs(det(sg.setting.R))
    end
...
# Enumerate every subgroup setting reachable by some maximal-subgroup descent
# path to the target group `symops` (expressed in `cryst`'s cell). Each result is
# a pair (subnum, P), where the tabulated string τ maps the parent ITA cell to
# the maximal-subgroup ITA cell, so P = inv(τ) ∘ setting maps `cryst`'s cell to
# that subgroup's ITA standard cell. `symops` mapped into a candidate's ITA
# setting is compared against the tabulated database group `std` as unordered
# sets (`same_symops`), with `issubset` for containment. When `symops` sits
# deeper than a maximal subgroup, we recurse into every maximal subgroup that
# contains it. Every descent step passes to a maximal t-subgroup, whose order is
# strictly smaller, so each path terminates on its own; the `seen` list only
# prunes intermediate groups reachable by multiple descent paths, avoiding
# exponential re-exploration of the subgroup DAG's shared nodes.
function subgroup_settings!(acc, seen, sgnum::Int, setting::SymOp, symops; atol)
    any(((n, s),) -> n == sgnum && isapprox(s, setting; atol), seen) && return acc
    push!(seen, (sgnum, setting))
    for (subnum, str) in t_subgroups[sgnum]
        P = inv(parse_op(str)) * setting
        std = SymOp.(Spglib.get_symmetry_from_database(standard_setting[subnum])...)
        mapped = [P * s * inv(P) for s in symops]
        if same_symops(mapped, std; atol)
            push!(acc, (subnum, P))
        elseif issubset(mapped, std; atol)
            subgroup_settings!(acc, seen, subnum, P, symops; atol)
        end
    end
    return acc
end

# Build a new `Crystal` whose spacegroup is the t-type (translationengleiche)
# subgroup of `cryst` generated by the reduced symmetry operations `symops`. The
# provided `symops` must be a subset of `cryst.sg.symops` and must form a
# complete t-subgroup (identity, rotations, and any retained lattice-centering
# translations). The lattice vectors are unchanged, so the new `sg.setting` maps
# the (possibly enlarged) cell to the ITA standard setting of the subgroup.
#
# The setting is ambiguous: distinct descent paths through `t_subgroups` reach
# the subgroup's ITA cell in different orientations, and within a fixed cell any
# subgroup symop `s` gives another valid setting `s ∘ P`. `conventionalize_setting`
# resolves the latter (symop) freedom; we apply it to each path setting and keep
# the best-scoring result. This favors clean settings when a path offers one
# (e.g. breaking an axis-aligned cell to P1 keeps the axes aligned), but it does
# not canonicalize: the full setting freedom is the subgroup's affine normalizer,
# and the descent paths realize only part of it, so an unexplored normalizer
# element could yield a lower-scoring (e.g. 90° rather than 120°) rotation. Every
# returned setting is nonetheless a correct map to the subgroup's ITA cell; only
# the cosmetic choice among equivalent settings is affected, which no downstream
# code depends on.
function subgroup_crystal(cryst::Crystal, symops::AbstractVector{SymOp}; symprec=1e-8)
    issubset(symops, cryst.sg.symops) || error("Provided `symops` are not a subset of `cryst.sg.symops`.")

    settings = subgroup_settings!(Tuple{Int,SymOp}[], Set(), cryst.sg.number, cryst.sg.setting, symops; atol=symprec)
    isempty(settings) && error("Provided `symops` do not form a t-subgroup of spacegroup $(cryst.sg.number).")

    # All paths reach the same abstract group, hence the same ITA spacegroup type.
    subnum = first(first(settings))
    @assert all(==(subnum), first.(settings))

    candidates = [conventionalize_setting(cryst.latvecs, P, subnum) for (_, P) in settings]
    P = candidates[argmin(score_setting.(candidates; latvecs=cryst.latvecs))]

    # Recompute equivalence classes: atoms are grouped into orbits under the
    # reduced `symops`, which may split parent classes into finer ones.
    classes = zeros(Int, natoms(cryst))
    for i in eachindex(classes)
        iszero(classes[i]) || continue
        c = maximum(classes) + 1
        for s in symops
            x = wrap_to_unit_cell(transform(s, cryst.positions[i]))
            j = findfirst(y -> is_periodic_copy(x, y; tol=symprec), cryst.positions)
            classes[j] = c
        end
    end

    sg = Spacegroup(collect(symops), spacegroup_label(standard_setting[subnum]), subnum, P)
    return Crystal(nothing, cryst.latvecs, cryst.recipvecs, cryst.positions, cryst.types, classes, sg)
end
