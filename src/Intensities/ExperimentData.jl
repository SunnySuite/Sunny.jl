    ...
    JLD2.jldopen(filename, "r") do file
        ...
        for (i, name) in enumerate(axes_names)
            ...
            if std(diff(data_dims[i])) > 1e-4 * binwidth[i]
                printstyled(
                    "Warning possible non-uniform binning: mean width = $(binwidth[i]),  std width = $(std(diff(data_dims[i])))";
                    color=:yellow,
                )
            end
...
function parse_long_name(long_name)
    # You're welcome
    found = match(
        r"\[(|0|(?:-?[0-9]?(?:\.[0-9]+)?))[HKL]?,(|0|(?:-?[0-9]?(?:\.[0-9]+)?))[HKL]?,(|0|(?:-?[0-9]?(?:\.[0-9]+)?))[HKL]?\]",
        long_name,
    )
    if isnothing(found)
        return nothing
    end
    return map(x -> if isempty(x)
        1.0
    elseif x == "-"
        -1.0
    else
...
function quick_view_nxs(filename, keep_ax)
    ...
    return (bcs[keep_ax[1]], bcs[keep_ax[2]], int_signal)
end
