...
function sphere_points(n)
    ...
    return Vec3.(spherical_to_cartesian.(plane_to_sphere.(planar_fib_points(n))))
end
...
"""
function powder_average_binned(
    sc::SampledCorrelations,
    radial_binning_parameters,
        ...
        for (iω, ω) in enumerate(ωvals)
            ...
            if 1 <= rbin <= r_bin_count
                ...
                if isnothing(integrated_kernel) # `Delta-function energy' logic
                    ...
                    for ωbin_other in 1:ω_bin_count
                        ...
                        fraction_in_bin =
                            integrated_kernel(b - ω) - integrated_kernel(a - ω)
