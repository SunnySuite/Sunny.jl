...
function System(crystal::Crystal, moments::Vector{Pair{Int, Moment}}, mode::Symbol;
    ...
    param_map = Dict{String, ParamGroup}()
    ewald = nothing
    ...
    ret = System(nothing, mode, crystal, (1, 1, 1), Ns, κs, gs, interactions, param_map, ewald,
...
function clone_system(sys::System{N}) where N
    (; origin, mode, crystal, dims, Ns, gs, κs, extfield, interactions_union,
        param_map, ewald, dipoles, coherents, rng) = sys
    ...
    param_map_clone = Dict{String, ParamGroup}((k, copy(v)) for (k, v) in param_map)
    ...
    ret = System(origin_clone, mode, crystal, dims, Ns, copy(κs), copy(gs),
                 interactions_clone, param_map_clone, ewald_clone, copy(extfield),
