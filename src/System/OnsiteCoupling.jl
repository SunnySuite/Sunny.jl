...
function anisotropy_renormalization(S)
    λ = [
        ...
        1 - (15 / 2) / S + (85 / 4) / S^2 - (225 / 8) / S^3 + (137 / 8) / S^4 -
        (15 / 4) / S^5,
    ] # k=6
...
function set_onsite_coupling!(sys::System, op, i::Int)
    is_homogeneous(sys) ||
        error("Use `set_onsite_coupling_at!` for an inhomogeneous system.")
...
function set_onsite_coupling!(sys::System, fn::Function, i::Int)
    ...
    return set_onsite_coupling!(sys, fn(S), i)
end
...
function energy_and_gradient_for_classical_anisotropy(s::Vec3, stvexp::StevensExpansion)
    ...
    dA_dz = (6Jz¹, 1)
    E +=
        (c2[1] * real(Jp²) + c2[5] * imag(Jp²))A[3] +
        (c2[2] * real(Jp¹) + c2[4] * imag(Jp¹))A[2] +
        c2[3] * A[1]
    dE_dz += (c2[2] * real(Jp¹) + c2[4] * imag(Jp¹))dA_dz[2] + c2[3] * dA_dz[1]
    dE_dJp +=
        (2 / 2) * (c2[1] * Jp¹ - im * c2[5] * Jp¹)A[3] + (1 / 2) * (c2[2] - im * c2[4])A[2]
    ...
    A = (35Jz⁴ - (30X)Jz² + (3X²), 7Jz³ - (3X)Jz¹, 7Jz² - (X), Jz¹, 1)
    dA_dz = (140Jz³ - (60X)Jz¹, 21Jz² - 3X, 14Jz¹, 1)
    E +=
        (c4[1] * real(Jp⁴) + c4[9] * imag(Jp⁴))A[5] +
        (c4[2] * real(Jp³) + c4[8] * imag(Jp³))A[4] +
        (c4[3] * real(Jp²) + c4[7] * imag(Jp²))A[3] +
        (c4[4] * real(Jp¹) + c4[6] * imag(Jp¹))A[2] +
        c4[5] * A[1]
    dE_dz +=
        (c4[2] * real(Jp³) + c4[8] * imag(Jp³))dA_dz[4] +
        (c4[3] * real(Jp²) + c4[7] * imag(Jp²))dA_dz[3] +
        (c4[4] * real(Jp¹) + c4[6] * imag(Jp¹))dA_dz[2] +
        c4[5] * dA_dz[1]
    dE_dJp +=
        (4 / 2) * (c4[1] * Jp³ - im * c4[9] * Jp³)A[5] +
    ...
    E +=
        (c6[1] * real(Jp⁶) + c6[13] * imag(Jp⁶))A[7] +
        ...
        c6[7] * A[1]
    dE_dz +=
        (c6[2] * real(Jp⁵) + c6[12] * imag(Jp⁵))dA_dz[6] +
