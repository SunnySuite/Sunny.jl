...
function parse_observables(N; observables, correlations)
    ...
    elseif correlations isa AbstractVector{Tuple{Int64,Int64}}
        ...
        correlations = [
            (observable_names[i], observable_names[j]) for (i, j) in correlations
        ]
    end
    ...
    return ObservableInfo(observables, observable_ixs, idxinfo)
end
...
function lookup_correlations(
    obs::ObservableInfo, corrs; err_msg=αβ -> "Missing correlation $(αβ)"
)
...
function all_observable_names(obs::ObservableInfo)
    observable_names = Dict(value => key for (key, value) in obs.observable_ixs)
    return [observable_names[i] for i in 1:length(observable_names)]
end
