    ...
    for ints in sys.interactions_union
        for coupling in ints.pair
            ...
            if !iszero(bilin)
                ...
                Si = reinterpret(
                    reshape, Sunny.CVec{3}, view(dipole_operators, :, :, :, bond.i)
                )
                Sj = reinterpret(
                    reshape, Sunny.CVec{3}, view(dipole_operators, :, :, :, bond.j)
                )
            ...
            if !iszero(biquad)
                Jij = (
                    isa(biquad, Float64) ? biquad * scalar_biquad_metric : biquad
                )::Union{Vec5,Mat5}
                Qi = reinterpret(
                    reshape, Sunny.CVec{5}, view(quadrupole_operators, :, :, :, bond.i)
                )
                Qj = reinterpret(
                    reshape, Sunny.CVec{5}, view(quadrupole_operators, :, :, :, bond.j)
                )
