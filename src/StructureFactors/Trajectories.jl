...
accumulated into the `StructureFactor`. All trajectory parameters are specified
in the [`StructureFactor`](@ref).

Note that this function will _not_ change the spin state in `sys`. 
"""
