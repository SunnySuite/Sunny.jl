...
"""
    add_trajectory!(sf::StructureFactor, sys::System)

When given a `StructureFactor` and a `System`, `add_trajectory` will use the
spin state contained in the `System` as the initial condition for a dynamical
trajectory. The correlations generated by the trajectory are calculated and
accumulated into the `StructureFactor`. All trajectory parameters are
specified in the [`StructureFactor`](@ref).

Note that this function will _not_ change the state of your `System`
"""
function add_trajectory!(sf::StructureFactor, sys::System)
