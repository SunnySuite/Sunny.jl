...
"""
struct Lattice
    lat_vecs     :: Matrix{Float64}     # Columns of these are the lat/basis vectors
    basis_vecs   :: Matrix{Float64}
    size         :: Vector{Int}
end
...
struct ReciprocalLattice
    lat_vecs     :: Matrix{Float64}     # Columns of this are the reciprocal lattice vectors
    size         :: Vector{UInt}
end
...
mutable struct ChargeSystem <: AbstractSystem
    lattice       :: Lattice
    sites         :: Array{Float64}
end
...
function _parse_lattice(config::Dict{String, Any}) :: Lattice
    try
        ...
        lat_vecs = hcat(lat_vecs...)
        basis_vecs = hcat(basis_vecs...)

        return Lattice(lat_vecs, basis_vecs, lattice_size) 
    catch err
        if isa(err, KeyError)
